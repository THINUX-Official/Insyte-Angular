...
export class UlDashboard implements OnInit {
  readonly permissions = AppPermissions;

  activeSection: UlSection = 'overview';
  ...
  get sidebarUserInfo(): DashboardUserInfo {
    return {
      username: this.loggedUser?.username || 'ul',
  ...
  get displayName(): string {
    return this.loggedUser?.nickname || this.loggedUser?.username || 'Unit Leader';
  }

  get roleLabel(): string {
    return this.loggedUser?.roles?.join(', ') || 'UL';
  }
  ...
  setActiveSection(section: UlSection): void {
  ...
  onLeadAction(event: { action: string; row: any }): void {
    ...
    console.log('UL lead action:', event.action, event.row);
  }
  ...
  updateLead(event: { id: number; payload: any }): void {
    this.dashboardService.updateLead(event.id, event.payload).subscribe({
      ...
      error: (error) => {
        console.error('UL lead update failed', error);
  ...
  private loadLoggedUser(): void {
    ...
    if (!username) {
      this.loggedUser = {
        username: 'ul',
        nickname: 'Unit Leader',
        email: '',
        roles: roles.length ? roles : ['UL']
      };
  ...
  private applyUlFilters(): void {
    const ulId = Number(this.loggedUser?.id || 0);

    this.teamUsers = this.users.filter(user => {
      const isIc = (user.roles || []).includes('IC');
      return isIc && Number(user.supervisorId) === ulId;
    });

    this.teamUserIds = this.teamUsers.map(user => Number(user.id));

    this.teamLeads = this.leads.filter(lead =>
      this.teamUserIds.includes(Number(lead.assignedUserId || lead.agentId || lead.userId))
    );
  ...
  private updateSummaryCards(): void {
    this.summaryCards = [
      ...
      },
      {
        ...
        value: this.getCompletedLeadCount(),
        icon: '✅',
        tone: 'purple'
      },
      {
  ...
  private buildLeadStatusChart(): void {
    ...
    this.leadStatusChartOption = {
      title: {text: 'Team Lead Status', left: 'center'},
  ...
  private buildTeamPerformanceChart(): void {
    ...
    this.teamPerformanceChartOption = {
      title: {text: 'Team Performance Score', left: 'center'},
  ...
  private buildPredictionChart(): void {
    ...
    this.predictionChartOption = {
      title: {text: 'Team Prediction Risk', left: 'center'},
      tooltip: {trigger: 'item'},
      legend: {bottom: 0},
      series: [
        {
