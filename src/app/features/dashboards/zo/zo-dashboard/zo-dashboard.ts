...
@Component({
  ...
  templateUrl: './zo-dashboard.html',
  styleUrls: ['./zo-dashboard.scss']
})
export class ZoDashboard implements OnInit {
  readonly permissions = AppPermissions;

  activeSection: ZoSection = 'overview';
  activeTeamTab: TeamTab = 'rm';
  ...
  get sidebarUserInfo(): DashboardUserInfo {
    return {
      username: this.loggedUser?.username || 'zo',
  ...
  get displayName(): string {
    return this.loggedUser?.nickname || this.loggedUser?.username || 'Zonal Manager';
  }

  get roleLabel(): string {
    return this.loggedUser?.roles?.join(', ') || 'ZO';
  }
  ...
  setActiveSection(section: ZoSection): void {
  ...
  onLeadAction(event: { action: string; row: any }): void {
    ...
    console.log('ZO lead action:', event.action, event.row);
  }
  ...
  updateLead(event: { id: number; payload: any }): void {
    this.dashboardService.updateLead(event.id, event.payload).subscribe({
      ...
      error: (error) => {
        console.error('ZO lead update failed', error);
  ...
  private loadLoggedUser(): void {
    ...
    if (!username) {
      this.loggedUser = {
        username: 'zo',
        nickname: 'Zonal Manager',
        email: '',
        roles: roles.length ? roles : ['ZO']
      };
  ...
  private applyZoFilters(): void {
    const zoId = Number(this.loggedUser?.id || 0);

    this.teamRmUsers = this.users.filter(user => {
      const isRm = (user.roles || []).includes('RM');
      return isRm && Number(user.supervisorId) === zoId;
    });
  ...
  private buildLeadStatusChart(): void {
    ...
    this.leadStatusChartOption = {
      title: {text: 'Zonal Lead Status', left: 'center'},
  ...
  private buildTeamPerformanceChart(): void {
    ...
    this.teamPerformanceChartOption = {
      title: {text: 'Zonal IC Performance', left: 'center'},
  ...
  private buildPredictionChart(): void {
    ...
    this.predictionChartOption = {
      title: {text: 'Zonal Prediction Risk', left: 'center'},
      tooltip: {trigger: 'item'},
      legend: {bottom: 0},
      series: [
        {
