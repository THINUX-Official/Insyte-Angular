...
@Component({
  ...
  styleUrls: ['./bm-dashboard.scss']
})
export class BmDashboard implements OnInit {
  readonly permissions = AppPermissions;

  activeSection: BmSection = 'overview';
  activeTeamTab: TeamTab = 'ul';
  ...
  get sidebarUserInfo(): DashboardUserInfo {
    return {
      username: this.loggedUser?.username || 'bm',
  ...
  get displayName(): string {
    return this.loggedUser?.nickname || this.loggedUser?.username || 'Branch Manager';
  }

  get roleLabel(): string {
    return this.loggedUser?.roles?.join(', ') || 'BM';
  }
  ...
  setActiveSection(section: BmSection): void {
  ...
  onLeadAction(event: { action: string; row: any }): void {
    ...
    console.log('BM lead action:', event.action, event.row);
  }
  ...
  updateLead(event: { id: number; payload: any }): void {
    this.dashboardService.updateLead(event.id, event.payload).subscribe({
      ...
      error: (error) => {
        console.error('BM lead update failed', error);
  ...
  private applyBmFilters(): void {
    const bmId = Number(this.loggedUser?.id || 0);

    this.teamUlUsers = this.users.filter(user => {
      const isUl = (user.roles || []).includes('UL');
      return isUl && Number(user.supervisorId) === bmId;
    });
  ...
  private buildLeadStatusChart(): void {
    ...
    this.leadStatusChartOption = {
      title: {text: 'Branch Lead Status', left: 'center'},
  ...
  private buildTeamPerformanceChart(): void {
    ...
    this.teamPerformanceChartOption = {
      title: {text: 'Branch IC Performance', left: 'center'},
  ...
  private buildPredictionChart(): void {
    ...
    this.predictionChartOption = {
      title: {text: 'Branch Prediction Risk', left: 'center'},
      tooltip: {trigger: 'item'},
      legend: {bottom: 0},
      series: [
        {
