...
@Component({
  ...
  templateUrl: './sh-dashboard.html',
  styleUrls: ['./sh-dashboard.scss']
})
export class ShDashboard implements OnInit {
  readonly permissions = AppPermissions;

  activeSection: ShSection = 'overview';
  activeTeamTab: TeamTab = 'zo';
  ...
  leadColumns: DashboardTableColumn[] = [
    {key: 'id', label: 'ID', type: 'number'},
    {key: 'name', label: 'Customer', fallbackKeys: ['customerName']},
    {key: 'nic', label: 'NIC'},
    {key: 'mobile', label: 'Mobile', fallbackKeys: ['phone', 'contactNumber']},
    {key: 'assignedUsername', label: 'IC'},
    {key: 'status', label: 'Status', type: 'badge'},
    {key: 'premium', label: 'Premium', type: 'number', fallbackKeys: ['expectedPremium']},
  ...
  get sidebarUserInfo(): DashboardUserInfo {
    return {
      username: this.loggedUser?.username || 'sh',
  ...
  get displayName(): string {
    return this.loggedUser?.nickname || this.loggedUser?.username || 'Sector Head';
  }

  get roleLabel(): string {
    return this.loggedUser?.roles?.join(', ') || 'SH';
  }
  ...
  setActiveSection(section: ShSection): void {
  ...
  onLeadAction(event: { action: string; row: any }): void {
    ...
    console.log('SH lead action:', event.action, event.row);
  }
  ...
  updateLead(event: { id: number; payload: any }): void {
    this.dashboardService.updateLead(event.id, event.payload).subscribe({
      ...
      error: (error) => {
        console.error('SH lead update failed', error);
  ...
  private loadLoggedUser(): void {
    ...
    if (!username) {
      this.loggedUser = {
        username: 'sh',
        nickname: 'Sector Head',
        email: '',
        roles: roles.length ? roles : ['SH']
      };
  ...
  private applyShFilters(): void {
    const shId = Number(this.loggedUser?.id || 0);

    this.teamZoUsers = this.users.filter(user => {
      const isZo = (user.roles || []).includes('ZO');
      return isZo && Number(user.supervisorId) === shId;
    });

    this.teamZoIds = this.teamZoUsers.map(user => Number(user.id));

    this.teamRmUsers = this.users.filter(user => {
      const isRm = (user.roles || []).includes('RM');
      return isRm && this.teamZoIds.includes(Number(user.supervisorId));
    });

    this.teamRmIds = this.teamRmUsers.map(user => Number(user.id));

    this.teamBmUsers = this.users.filter(user => {
      const isBm = (user.roles || []).includes('BM');
      return isBm && this.teamRmIds.includes(Number(user.supervisorId));
    });
    ...
    this.summaryCards = [
      {
        ...
        value: this.teamZoUsers.length,
        icon: '🌐',
        tone: 'blue'
      },
      {
        title: 'RM Team Members',
        value: this.teamRmUsers.length,
        icon: '👔',
        tone: 'green'
      },
      {
  ...
  private buildLeadStatusChart(): void {
    ...
    this.leadStatusChartOption = {
      title: {text: 'Sector Lead Status', left: 'center'},
  ...
  private buildTeamPerformanceChart(): void {
    ...
    this.teamPerformanceChartOption = {
      title: {text: 'Sector IC Performance', left: 'center'},
  ...
  private buildPredictionChart(): void {
    ...
    this.predictionChartOption = {
      title: {text: 'Sector Prediction Risk', left: 'center'},
      tooltip: {trigger: 'item'},
      legend: {bottom: 0},
      series: [
        {
